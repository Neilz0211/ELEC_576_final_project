...
python3 train.py \
    ...
    --learning-rate-refiner 0 \
    --checkpoint checkpoint/swintest/epoch-19.pth \
    --checkpoint-dir checkpoint/swintest/stage2 \
    --log-dir log/swintest/stage2 \
    --epoch-start 20 \
    --epoch-end 27
...
class Trainer:
    ...
    def parse_args(self):
        ...
        parser.add_argument('--model-variant', type=str, required=True,
                            choices=['mobilenetv3', 'resnet50', 'swin', 'swin3d'])
        parser.add_argument('--decoder-variant', type=str, default='recurrent')
        # Matting dataset
        ...
        parser.add_argument('--log-train-loss-interval', type=int, default=500)
    ...
    def init_model(self):
        ...
        self.model = MattingNetwork(self.args.model_variant, decoder_variant=self.args.decoder_variant,
                                    pretrained_backbone=True).to(self.rank)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            self.epoch_loss /= len(self.dataloader_lr_train)
            self.epoch_accuracy /= len(self.dataloader_lr_train)
            if self.rank == 0:
                self.writer.add_scalar(f'train_loss', self.epoch_loss, self.epoch)
                self.writer.add_scalar(f'train_accuracy', self.epoch_accuracy, self.epoch)
    ...
    def train_mat(self, true_fgr, true_pha, true_bgr, downsample_ratio, tag):
        ...
        if self.rank == 0 and self.step % self.args.log_train_loss_interval == 0:
            for loss_name, loss_value in loss.items():
                self.writer.add_scalar(f'train_{tag}_{loss_name}', loss_value, self.epoch)
    ...
    def validate(self):
        if self.rank == 0:
            ...
            self.model_ddp.eval()
            alpha_loss, total_loss, total_count = 0, 0, 0
            with torch.no_grad():
                with autocast(enabled=not self.args.disable_mixed_precision):
            ...
            avg_pha_loss = alpha_loss / total_count
            self.log(f'Validation set average loss: {avg_loss}')
            self.log(f'Validation set average accuracy: {avg_pha_loss}')
            self.writer.add_scalar('valid_loss', avg_loss, self.epoch)
            self.writer.add_scalar('valid_accuracy', avg_pha_loss, self.epoch)
